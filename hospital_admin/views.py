...
def admin_dashboard(request):
    ...
    if request.user.is_hospital_admin:
        ...
        total_doctor_count = Doctor_Information.objects.annotate(
            count=Count("doctor_id")
        )
        total_pharmacist_count = Pharmacist.objects.annotate(
            count=Count("pharmacist_id")
        )
        total_hospital_count = Hospital_Information.objects.annotate(
            count=Count("hospital_id")
        )
        total_labworker_count = Clinical_Laboratory_Technician.objects.annotate(
            count=Count("technician_id")
        )
        ...
        sat_count = (
            Appointment.objects.filter(date=sat_date_str)
            .filter(Q(appointment_status="pending") | Q(appointment_status="confirmed"))
            .count()
        )
        sun_count = (
            Appointment.objects.filter(date=sun_date_str)
            .filter(Q(appointment_status="pending") | Q(appointment_status="confirmed"))
            .count()
        )
        mon_count = (
            Appointment.objects.filter(date=mon_date_str)
            .filter(Q(appointment_status="pending") | Q(appointment_status="confirmed"))
            .count()
        )
        tues_count = (
            Appointment.objects.filter(date=tues_date_str)
            .filter(Q(appointment_status="pending") | Q(appointment_status="confirmed"))
            .count()
        )
        wed_count = (
            Appointment.objects.filter(date=wed_date_str)
            .filter(Q(appointment_status="pending") | Q(appointment_status="confirmed"))
            .count()
        )
        thurs_count = (
            Appointment.objects.filter(date=thurs_date_str)
            .filter(Q(appointment_status="pending") | Q(appointment_status="confirmed"))
            .count()
        )
        fri_count = (
            Appointment.objects.filter(date=fri_date_str)
            .filter(Q(appointment_status="pending") | Q(appointment_status="confirmed"))
            .count()
        )
...
def patient_list(request):
    ...
    patients = Patient.objects.all()
    return render(
        request, "hospital_admin/patient-list.html", {"all": patients, "admin": user}
    )
...
def edit_emergency_information(request, pk):
    ...
    if request.method == "POST":
        form = EditEmergencyForm(request.POST, request.FILES, instance=hospital)
        if form.is_valid():
            form.save()
...
def create_report(request, pk):
    if request.user.is_labworker:
        ...
        tests = Prescription_test.objects.filter(prescription=prescription).filter(
        ...
        if request.method == "POST":
            ...
            html_message = render_to_string(
                "hospital_admin/report-mail-delivery.html", {"values": values}
            )
...
def edit_medicine(request, pk):
    ...
    return render(
        request,
        "hospital_admin/edit-medicine.html",
        {"medicine": medicine, "admin": user},
    )
    ...
    if request.user.is_hospital_admin:
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                messages.success(
                    request, "Clinical Laboratory Technician account was created!"
                )
...
def view_lab_worker(request):
    ...
    return render(
        request,
        "hospital_admin/lab-worker-list.html",
        {"lab_workers": lab_workers, "admin": user},
    )
...
def view_pharmacist(request):
    ...
    return render(
        request,
        "hospital_admin/pharmacist-list.html",
        {"pharmacist": pharmcists, "admin": user},
    )
...
def edit_lab_worker(request, pk):
    ...
    return render(
        request,
        "hospital_admin/edit-lab-worker.html",
        {"lab_worker": lab_worker, "admin": user},
    )
...
def edit_pharmacist(request, pk):
    ...
    return render(
        request,
        "hospital_admin/edit-pharmacist.html",
        {"pharmacist": pharmacist, "admin": user},
    )
...
def register_doctor_list(request):
    ...
    return render(
        request,
        "hospital_admin/register-doctor-list.html",
        {"doctors": doctors, "admin": user},
    )
...
def pending_doctor_list(request):
    ...
    return render(
        request,
        "hospital_admin/Pending-doctor-list.html",
        {"all": doctors, "admin": user},
    )
...
def admin_doctor_profile(request, pk):
    ...
    experience = Experience.objects.filter(doctor_id=pk).order_by(
        "-from_year", "-to_year"
    )
...
def accept_doctor(request, pk):
    ...
    html_message = render_to_string(
        "hospital_admin/accept-doctor-mail.html", {"values": values}
    )
    plain_message = strip_tags(html_message)
...
def reject_doctor(request, pk):
    ...
    html_message = render_to_string(
        "hospital_admin/reject-doctor-mail.html", {"values": values}
    )
    plain_message = strip_tags(html_message)
...
def pharmacist_dashboard(request):
    if request.user.is_authenticated:
        if request.user.is_pharmacist:
            ...
            total_medicine_count = Medicine.objects.annotate(
