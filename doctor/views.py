...
def doctor_dashboard(request):
    if request.user.is_authenticated:
        if request.user.is_doctor:
            ...
            today_appointments = (
                Appointment.objects.filter(date=current_date_str)
            ...
            next_days_appointment = (
                Appointment.objects.filter(date=next_date_str)
                .filter(doctor=doctor)
                .filter(
...
def doctor_profile(request, pk):
    ...
    experiences = Experience.objects.filter(doctor=doctor).order_by(
        "-from_year", "-to_year"
    )
...
def booking(request, pk):
    ...
    if request.method == "POST":
        ...
        transformed_date = datetime.datetime.strptime(date, "%m/%d/%Y").strftime(
            "%Y-%m-%d"
        )
        ...
        if message:
            ...
            html_message = render_to_string(
                "appointment-request-mail.html", {"values": values}
            )
            plain_message = strip_tags(html_message)
...
def patient_profile(request, pk):
    if request.user.is_doctor:
        ...
        prescription = Prescription.objects.filter(doctor=doctor).filter(
            patient=patient
        )
...
def doctor_review(request, pk):
    ...
    if request.user.is_patient:
        ...
        if request.method == "POST":
            ...
            doctor_review = Doctor_review(
                doctor=doctor, patient=patient, title=title, message=message
            )
            doctor_review.save()
