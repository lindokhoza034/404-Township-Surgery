...
def generate_random_string():
    N = 8
    string_var = ""
    string_var = "".join(random.choices(string.ascii_uppercase + string.digits, k=N))
...
def ssl_payment_request(request, pk, id):
    ...
    post_body = {}
    post_body["total_amount"] = (
        appointment.doctor.consultation_fee + appointment.doctor.report_fee
    )
    ...
    post_body["fail_url"] = request.build_absolute_uri(reverse("ssl-payment-fail"))
    post_body["cancel_url"] = request.build_absolute_uri(reverse("ssl-payment-cancel"))
    ...
    post_body["cus_city"] = "Johannesburg"
    post_body["cus_country"] = "Gauteng"
    ...
    if status == "VALID":
        ...
        if payment_type == "appointment":
            ...
            html_message = render_to_string(
                "appointment_mail_payment_template.html", {"values": values}
            )
            ...
            # # Mailtrap
            patient_email = payment.patient.email
            patient_name = payment.patient.username
            patient_username = payment.patient.username
            patient_phone_number = payment.patient.phone_number
            ...
            for i in order_cart:
                ...
                pres_test = Prescription_test.objects.filter(
                    prescription=prescription
                ).filter(test_info_id=test_id)
            ...
            html_message = render_to_string(
                "test_mail_payment_template.html", {"values": values}
            )
            plain_message = strip_tags(html_message)
            ...
            html_message = render_to_string(
                "pharmacy_mail_payment_template.html", {"values": values}
            )
            plain_message = strip_tags(html_message)
